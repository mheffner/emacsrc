...
;; connections.
(defun url-http-debug (&rest args)
  (if url-http-debug
      (save-excursion
	(set-buffer (get-buffer-create "*HTTP-DEBUG*"))
	(goto-char (point-max))
...
(defun url-http-handle-authentication (proxy)
  ...
  (let ((auth (or (mail-fetch-field (if proxy "proxy-authenticate" "www-authenticate"))
	...
	(url-basic-auth-storage url-basic-auth-storage))

    ;; Cheating, but who cares? :)
    (if proxy
	(setq url-basic-auth-storage url-proxy-basic-authentication))
	...
	(case url-http-response-status
	  ...
	  ((301 302 307)
	   ...
	   (if (member url-http-method '("HEAD" "GET"))
	       ...
	       nil
	     (if (y-or-n-p (format "Honor redirection for method `%s'? "
				   url-http-method))
		 ;; User said it was ok, it is their neck after all
		 nil
	       (url-mark-buffer-as-dead (current-buffer))
	       (error "Refusing redirection for method `%s'"
		      url-http-method))))
	  (303
	  ...
	  (otherwise
	   ...
	   nil))
	(if (not redirect-uri)
	    ;; Redirection was cancelled for some reason.
	    nil
...
(defun url-http-activate-callback ()
  ...
  (declare (special url-callback-function url-callback-arguments))
    ...
    (let ((read-next-chunk t)
	  ...
	  (if (> nd (+ url-http-chunked-start url-http-chunked-length))
		...
		(url-http-debug "Got to the end of chunk #%d!"
				url-http-chunked-counter)
		(goto-char (+ 1 url-http-chunked-start
			      url-http-chunked-length)))
