...
(eval-when-compile
  ...
  (defvar *internal*)
  (defvar lc-ltn1)
  (defvar charset-latin-iso8859-1))
...
(defalias 'mule-coding-system-p
  (cond ((fboundp 'coding-system-p) 'coding-system-p)
	...
	(let ((l w3-url-domain-language-environment-alist))
	  (while l
		...
		(setq coding-system
		      ...
		      l nil)
	      (setq l (cdr l))))))
    ;; Next, check if the data are from some domain which prefers a
...
(defun mule-find-charset-region (beg end &optional table)
  (case mule-sysdep-version
    ...
    ((2.4 3.0 4.0 4.1 5.0) (find-charset-region beg end table))
...
(defun mule-find-coding-system (sys)
  (case mule-sysdep-version
    ((2.3 2.4) nil)
    ((3.0 4.0 4.1 5.0) (if (get sys 'coding-system) sys nil))
